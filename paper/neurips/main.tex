...
%% NOTE: This file expects the official NeurIPS 2026 style file.
%% Drop `neurips_2026.sty` into this directory before compiling.
%% Use `\usepackage[final]{neurips_2026}` for the camera-ready version.
...
\author{%
  Aviral Srivastava \\
  Amazon / Independent Researcher \\
  Seattle, WA, USA \\
  \texttt{aviralsr@example.org}
}
...
\paragraph{E1: Proof of concept.} On $50$ HarmBench items and a single fixed safety system prompt, we verify that ARA produces $k{=}3$ tokens that (a) reduce SAS by $\geq 70\%$ of baseline and (b) elicit non-refusal completions. E1 establishes that attention-only optimization is viable.

\paragraph{E2: ASR vs.\ budget.} We sweep $k \in \{1,2,3,5,8\}$ and report attack success rate (ASR) judged by HarmBench classifiers. We expect ARA to reach $>60\%$ ASR at $k{=}3$ across models and to plateau by $k{=}8$, tracking the phase-transition prediction.

\paragraph{E3: Filter evasion.} We pass successful ARA prompts through Llama Guard \cite{inan2023llamaguard} and report the fraction that evade flagging. Because ARA suffixes carry no harmful semantics -- by construction the optimization pressure is on attention geometry, not output content -- we expect $>80\%$ evasion, compared to ${\sim}10$--$30\%$ for GCG suffixes whose gibberish or affirmative-prefix tokens are often flagged.

\paragraph{E4: Cross-model transferability.} We construct ARA tokens on Llama-3 and apply them directly to Mistral, Gemma-2, and Llama-2. We report transfer ASR and compare against Proposition~\ref{prop:transfer}.

\paragraph{E5: Phase transition.} We linearly interpolate adversarial-token embeddings between benign initialization and the optimized ARA solution, sweeping the resulting SAS from $\SAS_0$ down to near-zero. We plot $R(\SAS)$ and fit the sigmoid predicted by Theorem~\ref{thm:phase}. We extract $S^*$ and width $w$ per model.

\paragraph{E6: Defense.} We evaluate ABC (Section~\ref{sec:defense}) against ARA and report ASR reduction plus MT-Bench utility. See Figures~\ref{fig:asr_budget}--\ref{fig:pareto}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/asr_vs_budget.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/phase_transition.pdf}
...
ARA exposes a previously unquantified failure mode of deployed LLM safety alignment: models may not attend to their own safety instructions, and an adversary can make this happen with $1$--$3$ semantically neutral tokens that slip past guardrail classifiers. Publishing the attack accelerates awareness of this vulnerability; to balance dual-use risk, we ship the Attention Budget Constraint (ABC) defense alongside the attack, conduct $90$-day coordinated disclosure with affected model vendors, restrict our evaluation to the standard HarmBench red-team set, and withhold specific adversarial token strings from the main text. Longer term, our results argue that safety alignment should supervise attention allocation, not only output distributions, and that the field should adopt attention-level metrics such as SAS as standard evaluation artefacts.
